...
void OutputWriter::WriteCSV() {
    ...
    for (unsigned int i = 0; i < GI::TCX(); i++){
        ...
        csv_file << block_.primitive_buffer_[PrimeStatePool::Density][i] << ",";
        csv_file << block_.primitive_buffer_[PrimeStatePool::VelocityX][i] << ",";
        csv_file << block_.primitive_buffer_[PrimeStatePool::Pressure][i] << ",";
        csv_file << block_.conservative_buffer_[ConservativePool::Energy][i] << std::endl;
    }
    csv_file.close();
}